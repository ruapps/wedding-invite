...
const MAX_BYTES = 100 * 1024 * 1024; // 40 MB
...
export default function CongratsSection() {
  ...
  async function validateAndUpload(blob) {
    ...
    if (blob.size > MAX_BYTES) {
      setError(
        `File too large: ${(blob.size / (1024 * 1024)).toFixed(
          1
        )} MB. Max allowed ${MAX_BYTES / (1024 * 1024)} MB.`
      );
      return false;
    }
    ...
    if (duration > MAX_SECONDS) {
      setError(
        `Video too long: ${Math.ceil(duration)}s. Max allowed ${MAX_SECONDS}s.`
      );
      return false;
    }
  ...
  async function uploadClip(blob, durationSeconds) {
    ...
    try {
      const filePath = `congrats/${Date.now()}-${Math.random()
        .toString(36)
        .slice(2, 9)}.webm`;
  ...
  return (
    <div style={{ marginTop: 18 }}>
      ...
      <p style={{ color: "#555" }}>
        Record a short video message (will be uploaded and shown below with your
        name). Max {MAX_SECONDS}s, {Math.round(MAX_BYTES / (1024 * 1024))}MB.
      </p>
      ...
      >
        <input
          ref={nameRef}
          placeholder="Your name (will appear with the video)"
          style={{
            padding: 8,
            borderRadius: 6,
      ...
      >
        ...
        {clips.length === 0 && (
          <div style={{ color: "#666" }}>
            No guest videos yet — be the first to record!
          </div>
        )}
      </div>
    </div>
